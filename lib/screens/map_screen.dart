...
//                ),
  ...
  createMarkers() async {
    ...
    for (int i = 0; i < snap.data['entries'].length; i++) {
      ...
      allMarkers.add(new Marker(
        ...
        position: LatLng(location.data["coords"].latitude,
            location.data["coords"].longitude),
        onTap: () {
          _markerPressed(location.data["title"], location.data["locality"], location.data["imageUrl"]);
        },
      ));
    }
    String st = "String";
    ...
    allMarkers.add(new Marker(
      ...
      onTap: () {
        _markerPressed("Current Location", "[${currentPos.latitude},${currentPos.longitude}]", null);
      },
      icon: icon,
    ));
  }
