...
class _ViewMemoryScreenState extends State<ViewMemoryScreen> {
  ...
  void initState() {
    ...
    setState(() {
      nowsong = new Track(trackTitle: widget.songtitle, trackAuthor: widget.songartist);
      _search = musicStore.search(nowsong.trackTitle);
    });

  }

  Widget _nowPlayingPanel() {
    return SoundPlayerUI.fromTrack(Track(trackTitle: widget.songtitle, trackAuthor: widget.songartist));
  ...
  // Duration slider_current_position = 0.0;
  // Duration max_duration = 1.0;
  //
  // void startPlayer(String uri) async {
  //   Duration duration = await flutterSound.startPlayer(fromURI: uri);
  //   await flutterSound.setVolume(1.0);
  //   //print('startPlayer: $path');
  //
  //   try {
  //     _playerSubscription = flutterSound.onProgress.listen((e) async {
  //       if (e != null) {
  //         slider_current_position = e.position;
  //         max_duration = e.duration;
  //
  //         final remaining = e.duration.;
  //
            ...
            if (snapshot.hasData &&
              ...
              for (Song song in songs) {
                ...
                if (song.title.toLowerCase() ==
                    nowsong.trackTitle.toLowerCase() &&
                    song.artistName.toLowerCase() ==
                        nowsong.trackAuthor.toLowerCase()) {
                  sons.add(song);
                }
              }
